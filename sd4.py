...
# SIFT matching
MIN_MATCH_COUNT = 8         # Minimum good feature matches to consider
...
def enhance_image(gray_img):
    """Apply CLAHE + sharpening to boost features on smooth symbols."""
    clahe = cv2.createCLAHE(clipLimit=3.0, tileGridSize=(8, 8))
    enhanced = clahe.apply(gray_img)
    # Sharpen to bring out edges
...
def load_templates(template_dir):
    ...
    for f in files:
        ...
        # Keep templates at higher resolution for more features
        h, w = img.shape
        if max(h, w) > 700:
            s = 700 / max(h, w)
        ...
        kp, des = sift.detectAndCompute(img_enhanced, None)
