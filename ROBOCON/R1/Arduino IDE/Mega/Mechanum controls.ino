// ================================================================
//  MECANUM WHEEL CAR  —  v2.3  (+ pneumatic actuators via L1/R1)
...
void setup() {
    ...
    pinMode(PNEU_L1_DIR, OUTPUT); pinMode(PNEU_L1_PWM, OUTPUT);
    pinMode(PNEU_R1_DIR, OUTPUT); pinMode(PNEU_R1_PWM, OUTPUT);
    setMotor(PNEU_L1_DIR, PNEU_L1_PWM, 0);
    setMotor(PNEU_R1_DIR, PNEU_R1_PWM, 0);
...
void loop() {
    ...
    if (!activeComms) {
        if (targetLF != 0 || targetLB != 0 || targetRF != 0 || targetRB != 0
            || pneuL1 != 0 || pneuR1 != 0) {
            Serial.println(F("[TIMEOUT] Comms lost — zeroing targets + retracting pneumatics"));
        }
        targetLF = targetLB = targetRF = targetRB = 0;
