...
def main():
    ...
    # SIFT detector (fewer features for live speed)
    ...
    def detection_worker():
        """Background thread: runs SIFT detection without blocking camera."""
        nonlocal c_name, c_inliers, c_boundary
    ...
    while True:
        ...
        with lock:
            if not worker_busy[0]:
                ...
                latest_gray[0] = cv2.cvtColor(small, cv2.COLOR_BGR2GRAY)

        # Read detection results (thread-safe)
        with lock:
            local_name = c_name
