// ================================================================
//  MECANUM WHEEL CAR  —  v2.4  (WiFi Wireless Version)
//  Hardware  : Arduino Mega 2560 + Rhino MDD20Amp ×3 + 4 DC Motors
//              + 2× pneumatic solenoid valves (CA-DCV-SSV-5218)
//              Pneumatics driven via 3rd MDD20Amp (DIR+PWM per channel)
//  Interface : PS4 via Python → WiFi → ESP32 → Serial1 (pins 18/19)
//
//  CHANGES FROM V8 (WIRED):
//    • Data input: Serial (USB) → Serial1 (pins 19 TX, 18 RX)
//    • Serial (USB) is now DEBUG-ONLY (view in Arduino Serial Monitor)
//    • TEST and PING commands work from BOTH Serial (USB) and Serial1
//    • ALL other features are 100% identical to V8
//
//  Serial packet received from ESP32 (originated from Python):
//    <LX,LY,RX,L1,R1>\n
...
//
//  Features (ALL preserved from V8):
...
//    • L1/R1 → pneumatic solenoid control (instant on/off)
//
//  WIRING — ESP32 to Arduino Mega:
...
// Pneumatic solenoid valve pins (3rd MDD20Amp)
//   CH1 → L1 pneumatic (CA-DCV-SSV-5218)
//   CH2 → R1 pneumatic (CA-DCV-SSV-5218)
#define PNEU_L1_DIR 28
#define PNEU_L1_PWM 7

#define PNEU_R1_DIR 30
#define PNEU_R1_PWM 8
...
// ================================================================
#define MAX_SPEED                                                              \
  60 // ← change ONLY this during testing
     //   bench test   →  60
     //   first floor  →  100
     //   normal use   →  180
     //   full speed   →  255
...
// Pneumatic state (0 = retracted, 1 = extended)
int pneuL1 = 0;
int pneuR1 = 0;
...
void stopAll() {
  ...
  pneuL1 = 0;
  pneuR1 = 0;
  setMotor(PNEU_L1_DIR, PNEU_L1_PWM, 0);
  setMotor(PNEU_R1_DIR, PNEU_R1_PWM, 0);
}
...
int rampStep(int current, int target) {
  ...
  bool sameDir = (current > 0 && target > 0) || (current < 0 && target < 0);
...
//  and prints a detailed status block to Serial (USB debug).
//
...
// ================================================================
void processGamepad(int joyX, int joyY, int rJoyX, int btnL1, int btnR1) {
  ...
  // Write pneumatic state — instant on/off via MDD20Amp, no ramping
  pneuL1 = btnL1;
  pneuR1 = btnR1;
  setMotor(PNEU_L1_DIR, PNEU_L1_PWM, pneuL1 ? 255 : 0);
  setMotor(PNEU_R1_DIR, PNEU_R1_PWM, pneuR1 ? 255 : 0);

  // ----------------------------------------------------------
  //  ===  DEBUG OUTPUT (printed to USB Serial Monitor)  ===
  // ----------------------------------------------------------
  Serial.println(F("================================================"));
  Serial.print(F("  MODE  : "));
  Serial.println(mode);
  ...
  // Pneumatic status
  Serial.print(F("  PNEUM  L1="));
  Serial.print(pneuL1 ? "ON " : "off");
  Serial.print(F("  R1="));
  Serial.println(pneuR1 ? "ON " : "off");
  ...
  if (millis() - lastPacketTime > SERIAL_TIMEOUT / 2) {
    Serial.println(F("  [WARN] No packet recently — check WiFi connection"));
  }
...
// ================================================================
//  SERIAL PARSER  —  reads from Serial1 (ESP32 bridge)
...
//  from BOTH Serial (USB) and Serial1 (ESP32).
// ================================================================
...
// Line buffer for plain-text commands from Serial1
char cmdBuf1[BUF_SIZE];
int cmdIdx1 = 0;

// Line buffer for plain-text commands from Serial (USB debug)
char cmdBuf0[BUF_SIZE];
int cmdIdx0 = 0;

// ----------------------------------------------------------------
//  readPacket()  —  reads data from Serial1 (ESP32 WiFi bridge)
//  Returns true when a complete <...> gamepad packet is received.
// ----------------------------------------------------------------
bool readPacket() {
  while (Serial1.available()) {
    char c = Serial1.read();

    // ---- structured packet parser  <LX,LY,RX,L1,R1> ----
    if (c == '<') {
      inPacket = true;
      rxIdx = 0;
      cmdIdx1 = 0; // reset cmd buffer on new packet
    } else if (c == '>' && inPacket) {
      rxBuf[rxIdx] = '\0';
      inPacket = false;
      rxIdx = 0;
    ...
    // ---- plain-text command parser from Serial1 (newline terminated) ----
    else if (!inPacket) {
      if (c == '\n' || c == '\r') {
        if (cmdIdx1 > 0) {
          cmdBuf1[cmdIdx1] = '\0';
          ...
          if (strcmp(cmdBuf1, "TEST") == 0) {
            motorSelfTest();
          } else if (strcmp(cmdBuf1, "PING") == 0) {
            Serial1.println(F("PONG")); // reply back through ESP32
            Serial.println(F("[PING from WiFi → PONG sent]"));
          }
        }
      } else if (cmdIdx1 < BUF_SIZE - 1) {
        cmdBuf1[cmdIdx1++] = c;
      }
    }
  }
...
// ----------------------------------------------------------------
//  checkUSBCommands()  —  reads plain-text commands from Serial (USB)
//  Allows TEST and PING from the Arduino Serial Monitor directly.
// ----------------------------------------------------------------
void checkUSBCommands() {
  while (Serial.available()) {
    ...
    if (c == '\n' || c == '\r') {
      if (cmdIdx0 > 0) {
        ...
        if (strcmp(cmdBuf0, "TEST") == 0) {
          motorSelfTest();
        } else if (strcmp(cmdBuf0, "PING") == 0) {
          Serial.println(F("PONG")); // reply on USB
        }
      }
    } else if (cmdIdx0 < BUF_SIZE - 1) {
      cmdBuf0[cmdIdx0++] = c;
    }
  }
}
...
// ================================================================
void setup() {
  // USB Serial — DEBUG OUTPUT ONLY (view in Arduino Serial Monitor)
  Serial.begin(115200);

  // Serial1 — DATA INPUT from ESP32 WiFi bridge (pins 19=TX1, 18=RX1)
  Serial1.begin(115200);

  Serial.println(F("================================================"));
  Serial.println(F("  MECANUM CAR  v2.4  —  WiFi Wireless Version"));
  Serial.println(F("  Arduino Mega 2560"));
  Serial.println(F("================================================"));
  Serial.println(F("  DATA  : Serial1 (pin 18 RX ← ESP32 TX)"));
  Serial.println(F("  DEBUG : Serial  (USB) — this monitor"));
  Serial.println(F("------------------------------------------------"));
  ...
  Serial.println(F("------------------------------------------------"));
  Serial.println(F("  Serial commands (type in this monitor + Enter):"));
  ...
  // Pneumatic solenoid pins (3rd MDD20Amp)
  pinMode(PNEU_L1_DIR, OUTPUT);
  pinMode(PNEU_L1_PWM, OUTPUT);
  pinMode(PNEU_R1_DIR, OUTPUT);
  pinMode(PNEU_R1_PWM, OUTPUT);
  setMotor(PNEU_L1_DIR, PNEU_L1_PWM, 0);
  setMotor(PNEU_R1_DIR, PNEU_R1_PWM, 0);
  ...
  Serial.println(F("Ready. Waiting for data from ESP32 WiFi bridge..."));
  Serial.println(F("================================================"));
}
...
void loop() {
  ...
  // Step 1 — parse incoming serial packet from Serial1 (ESP32)
  if (readPacket()) {
    int lx, ly, rx, l1btn, r1btn;
    if (sscanf(rxBuf, "%d,%d,%d,%d,%d", &lx, &ly, &rx, &l1btn, &r1btn) == 5) {
      lastPacketTime = millis();
      processGamepad(lx, ly, rx, l1btn, r1btn);
    } else {
  ...
  // Step 1b — check for commands from USB Serial Monitor
  checkUSBCommands();
  ...
  if (!activeComms) {
    if (targetLF != 0 || targetLB != 0 || targetRF != 0 || targetRB != 0 ||
        pneuL1 != 0 || pneuR1 != 0) {
      Serial.println(
          F("[TIMEOUT] Comms lost — zeroing targets + retracting pneumatics"));
    }
    targetLF = targetLB = targetRF = targetRB = 0;
    // Safety retract pneumatics on timeout
    pneuL1 = 0;
    pneuR1 = 0;
    setMotor(PNEU_L1_DIR, PNEU_L1_PWM, 0);
    setMotor(PNEU_R1_DIR, PNEU_R1_PWM, 0);
